...
static void handleMovement(int offsetX, int offsetY)
{
	...
	if (fstField == WM_FREE || fstField == WM_GOAL) {
		canMove = GL_TRUE;
		g_gamestate.lastSandbagX = -1;
		g_gamestate.lastSandbagY = -1;
		...
		canMove = GL_TRUE;
		g_gamestate.lastSandbagX = g_gamestate.playerX + offsetX * 2;
		g_gamestate.lastSandbagY = g_gamestate.playerY + offsetY * 2;
	}
...
void updateLogic(double interval)
{
	...
	if (!g_gamestate.showHelp && g_gamestate.stage == stageRunning)
	{
		...
		if (g_gamestate.playerCooldown > 0.0f) 
		{
...
void movePlayer(PlayerDirection direction)
{
	if (!g_gamestate.showHelp && 
		...
		g_gamestate.playerCooldown <= 0.0f) {
		g_gamestate.lastDirection = direction;
		switch (direction)
		{
			case dirDown:
				handleMovement(0,  1);
	...
	for (int y = 0; y < LEVELSIZE; y++)
	{
		for (int x = 0; x < LEVELSIZE; x++)
		{
			...
			if (levelSource[y][x] == WM_START) {
				...
				g_gamestate.level[y][x] = WM_FREE;
				g_gamestate.lastDirection = dirDown;
			}
		}
	}
...
void changeCameraOrientation(GLfloat deltaRadius, GLfloat deltaPolar, GLfloat deltaAzimuth)
{
	...
	if (g_gamestate.camera.polarAngle < 0.0f) {
		g_gamestate.camera.polarAngle += 360.0f;
	}

	if (g_gamestate.camera.polarAngle > 360.0f) {
		g_gamestate.camera.polarAngle -= 360.0f;
	}

	if (g_gamestate.camera.azimuthAngle < 5.0f) {
		g_gamestate.camera.azimuthAngle = 5.0f;
	}

	if (g_gamestate.camera.azimuthAngle > 90.0f) {
		g_gamestate.camera.azimuthAngle = 90.0f;
	}
}
